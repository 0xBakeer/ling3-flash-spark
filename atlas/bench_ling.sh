...
# THINKING=on|off. Ling's chat template is binary (no effort levels) and its
# DEFAULT is on, so `on` is the model's intended mode and matches every public
# thinking-on number. The server is started with --default-chat-template-kwargs,
# so this is a genuine server config and is recorded as such in args.
THINKING="${THINKING:-off}"
if [[ "$THINKING" == "on" ]]; then TK='{"enable_thinking": true}'; else TK='{"enable_thinking": false}'; fi
...
for W in "${WORKLOADS[@]}"; do
  ...
  uv run atlas-bench packet --cell "$CELL" --workload "$W" \
    ...
    --arg reasoning-parser=ling3 --arg tool-call-parser=ling3 --arg trust-remote-code=true \
    --arg fp8-lm-head=online \
    --request temperature=0 --request seed=42 --request timeout_s=3600 \
    --arg "default-chat-template-kwargs=$TK" \
...
PY
  uv run atlas-bench run --spec "packets/ling.$W.json" --base-url "$BASE" --out ../results --login "$LOGIN" \
     >"logs/ling.$W.run.log" 2>&1 && touch "logs/ling.$THINKING.$W.done" || echo "!! run failed for $W (see logs/ling.$W.run.log)"
done
